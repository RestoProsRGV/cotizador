...
const SUGGESTION_MAP: Record<string, string[]> = {
  ...
  "DEM-FLOOD-CUT": ["DEM-BSBD-RM", "DEM-INSUL-RM", "DEM-CAVITY-DRILL"],
...
describe("suggestion rules — Flood Cut", () => {
  it("suggests Baseboard, Insulation, and Cavity Drill", () => {
    const suggestions = SUGGESTION_MAP["DEM-FLOOD-CUT"]!;
    expect(suggestions).toContain("DEM-BSBD-RM");
    expect(suggestions).toContain("DEM-INSUL-RM");
    expect(suggestions).toContain("DEM-CAVITY-DRILL");
...
describe("applyAll — add trigger + all suggestions", () => {
  ...
  it("does not duplicate already-selected items", () => {
    ...
    // Should have DW-RM, BSBD-RM (pre-existing), INSUL-RM
    expect(result.size).toBe(3);
  });
