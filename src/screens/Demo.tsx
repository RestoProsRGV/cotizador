        ...
        {loading ? (
          <div style={{ padding: "16px" }}>
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                style={{ height: "56px", backgroundColor: "var(--color-border)", marginBottom: "8px", borderRadius: "4px" }}
              />
            ))}
          </div>
        ) : areas.length === 0 ? (
          ...
          displayAreas.map((area, idx) => (
            <div key={area.id}>
              ...
              <DemoTab
                ...
                areaId={area.id}
                areaName={area.name}
                areaSf={area.length * area.width}
                areaPerimeter={2 * (area.length + area.width)}
              />
            </div>
          ))
        )}
